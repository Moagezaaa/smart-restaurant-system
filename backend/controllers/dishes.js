...
const addDish = asyncWrap(async (req, res) => {
    ...
    await pool.query("INSERT INTO dishes (name, description, price, is_available) VALUES (?, ?, ?, coalesce(?, true))", [name, description, price, is_available]);
...
const deleteDish = asyncWrap(async (req, res) => {
    ...
    res.status(200).json({ status: httpStatusText.SUCCESS, message: "Dish deleted successfully" });
})
